...
export const baseTransactionSchema = z.object({
  ...
  amount: z.number().positive({ error: "Amount must be positive" }).min(1),
  category: z.string().min(1, { error: "Category is required" }),
  date: z
    .union([z.iso.datetime({ message: "Invalid date string" }), z.date()])
...
export const bulkDeleteTransactionSchema = z.object({
  transactionIds: z
    .array(z.string().length(24, { error: "Invalid transaction ID format" }))
    .min(1, { error: "At least one transaction ID must be provided" }),
});

export const bulkTransactionSchema = z.object({
  transactions: z
    .array(baseTransactionSchema)
    .min(1, { error: "At least one transaction is required" })
    .max(300, { error: "Must not be more than 300 transactions" })
    .refine(
      (tsx) =>
        tsx.every((tx) => {
      ...
      { error: "Amount must be positive number and less than 1 billion" }
    ),
});
