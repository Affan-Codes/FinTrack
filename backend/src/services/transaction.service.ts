...
export const duplicateTransactionService = async (
  ...
  const duplicated = await Transaction.create({
...
export const bulkTransactionService = async (
  ...
  try {
    ...
    const result = await Transaction.bulkWrite(bulkOps, { ordered: true });
...
export const scanReceiptService = async (
  ...
  try {
    ...
    if (!data.amount || !data.date) {
      return { error: "Receipt is missing required information" };
    }
