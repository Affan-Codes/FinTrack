...
import { ReportSetting } from "../models/reportSetting.model";
...
import { convertToDollarUnit } from "../utils/formatCurrency";
import { format } from "date-fns";
import { genAI, genAIModel } from "../config/googleAi.config";
...
export const getAllReportsService = async (
  ...
  const [reports, totalCount] = await Promise.all([
    Report.find(query).skip(skip).limit(pageSize).sort({ createdAt: -1 }),
...
export const updateReportSettingService = async (
  ...
  const existingReportSetting = await ReportSetting.findOne({ userId });
...
function calculateSavingRate(totalIncome: number, totalExpenses: number) {
  ...
  return parseFloat(savingRate.toFixed());
}
